...
#define CULL_NONE	1		// 不剔除
#define CULL_FRONT	2		// 剔除正面
#define CULL_BACK	4		// 剔除背面
...
void device_draw_primitive(device_t* device,
	const vertex_t* v1,
	const vertex_t* v2,
	const vertex_t* v3)
{
	...
	int render_state = device->render_state;
	int cull_state = device->cull_state;
...
{
	device_t device;
	int render_states[] = { RENDER_STATE_TEXTURE, RENDER_STATE_COLOR, RENDER_STATE_WIREFRAME };
	int cull_states[] = { CULL_NONE, CULL_FRONT, CULL_BACK };
	int indicator_render = RENDER_STATE_TEXTURE;
	int indicator_cull = CULL_BACK;
	int kbhit = 0;
	int kf2hit = 0;
	float alpha = 1;
	float pos = 3.5;
	...
	TCHAR* title = _T("Mini3d (software render tutorial) - ")
		_T("Left/Right: rotation, Up/Down: forward/backward, Space: switch state, F2: switch cull");
	...
	init_texture(&device);
	device.render_state = indicator_render;
	device.cull_state = indicator_cull;
	...
	while (screen_exit == 0 && screen_keys[VK_ESCAPE] == 0)
	{
		...
		{
			if (hit_keys[VK_SPACE] == 0)
			{
				hit_keys[VK_SPACE] = 1;
				indicator_render = (indicator_render + 1) % 3;
				device.render_state = render_states[indicator_render];
			}
		}
		else
		{
		...
		if (screen_keys[VK_F2])
		{
			...
			{
				hit_keys[VK_F2] = 1;
				indicator_cull = (indicator_cull + 1) % 3;
				device.cull_state = cull_states[indicator_cull];
			}
		}
		else
		{
